...
begin
      ...
      while Pos <= Argument_Count loop
         ...
         if Arg'Length >= 2 and then Arg (1 .. 2) = "-P" then
            if Self.Project_File /= No_String then
               raise GPRname_Exception
                 with "only one -P switch may be specified";
         ...
         then
            ...
            if Arg'Length = 2 then
               ...
               Current_Section.Add_Excluded_Language_Pattern
                 (Language_Type'("Ada"),
                  Pattern_Type (Arg.all (3 .. Arg'Last)));
               ...
               begin
                  Read_Next_Arg
                    (Err_Mssg => "excluded pattern missing for language "
                                 & Lang);
                  Current_Section.Add_Excluded_Language_Pattern
                    (Language_Type (Lang), Pattern_Type (Arg.all));
               end;
   ...
   exception
      when E : others =>
         ...
         raise GPRname_Exception with Ada.Exceptions.Exception_Message (E);
   end Loop_Through_Args;
