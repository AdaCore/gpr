...
package body GPRinstall.Install is
   ...
   --  Suffix of the file the compiler writes beside the object when asked to
   --  save the preprocessed form of a source, with "-gnateG"
   ...
   is
         ...
         if not Options.Dry_Run then
            ...
            if Sym_Link then
               ...
               if From_Ver.Is_Defined then
                  if From_Ver.Exists then
                     declare
                        R : constant Boolean :=
                              GNATCOLL.OS.FSUtil.Remove_File
                                (String (From_Ver.Value))
                          with Unreferenced;
                     begin
                        null;
                     end;
      ...
      procedure Copy_Files is
                  ...
                  if Is_Windows_Host then
                     ...
                     Copy_File
                       (From     => Path_Name.Compose
                        ...
                        File     => Path_Name.Compose
                          (Link_Lib_Dir,
                           Library_Name.Value).Simple_Name,
                        To       => Lib_Dir,
                        Sym_Link => True);
                  end if;
                  pragma Warnings
         ...
         begin
            ...
            declare
               --  Install the preprocessed form of the source when it
               --  exists. The ".prep" suffix is dropped during the copy.
               --  A pre-processed file that exists in the filesystem, but that
               --  was not generated by the latest build, will not be used.

               Preprocessed : constant Path_Name.Object :=
                                (if View.Kind in With_Object_Dir_Kind
                                 then View.Object_Directory.Compose
                                        (Source.Simple_Name & Prep_Suffix)
                                 else Path_Name.Undefined);

               function Has_Preprocessed_Source return Boolean;
               --  Returns true if the preprocessed source needs to be copied
               --  instead of the original source.
               --  First, the pre-processed source must exist, then it must
               --  belong to the artifacts of the current project.
               --  Because the preprocessing is enabled with a compiler switch
               --  -gnateG, which can be done via the gprbuild command line,
               --  gprinstall may not have this information. In that case,
               --  actions signatures are checked instead.

               -----------------------------
               -- Has_Preprocessed_Source --
               -----------------------------

               function Has_Preprocessed_Source return Boolean is
               begin
                  if not Preprocessed.Is_Defined
                    or else not Preprocessed.Exists
                  then
                     return False;
                  end if;

                  declare
                     Art : constant GPR2.Build.Artifacts.Files.Object :=
                             GPR2.Build.Artifacts.Files.Create (Preprocessed);
                  begin
                     --  This database already contains the artifact when
                     --  the "-gnateG" is specified in the project attributes.

                     if Options.Tree.Artifacts_Database.Has_Artifact (Art)
                     then
                        return True;
                     end if;

                     --  Otherwise the switch was given to gprbuild alone,
                     --  and only the signature of the compilation contains
                     --  the information.

                     for Action of
                       Options.Tree.Artifacts_Database.Successors
                         (GPR2.Build.Artifacts.Source_Files.Create (Source))
                     loop
                        if Action in Compile.Ada.Object'Class then
                           declare
                              Act : Compile.Ada.Object'Class :=
                                      Compile.Ada.Object'Class (Action);
                              --  The action must be modifiable, so that its
                              --  signature can be loaded

                           begin
                              Act.Load_Signature (Check_Checksums => False);

                              if Act.Signature.Valid (Art) then
                                 return True;
                              end if;
                           end;
                        end if;
                  ...
                  return False;
               end Has_Preprocessed_Source;

            begin
               Copy_File
                 (From => (if Has_Preprocessed_Source
                           then Preprocessed
                           else Source),
                  To   => Sources_Dir,
      ...
      procedure Create_Project (Project : GPR2.Project.View.Object) is
         ...
         procedure Create_Users_Packages;
         --  Create user's defined packages
         ...
         ---------------------------
         -- Create_Users_Packages --
         ---------------------------

         procedure Create_Users_Packages is
            Predef_Pcks : GPR2.Containers.Package_Id_List
              renames GPR2.Project.Registry.Pack.Predefined_Packages;
         begin
            for P
              of Project.Packages (With_Defaults => False,
                                   With_Config   => False)
            loop
               if not Predef_Pcks.Contains (P) then
                  Content.Append ("   package " & Image (P) & " is");

                  for Attr of Project.Attributes
                    (P,
                     With_Defaults => False,
                     With_Config   => False)
                  loop
                     Content.Append ("      " & Attr.Image);
                  end loop;
                  Content.Append ("   end " & Image (P) & ";");
                  Add_Empty_Line;
               end if;
            end loop;
         end Create_Users_Packages;
      ...
      begin
         ...
         if Project_Exists and then Generated then
            ...
            --  Project does not exist, or it exists but was not generated by
