...
def process_jsons(json_def, json_conf, input_filename):
    ...
    def get_package_to_display(package):
        ...
        if len(res) == 0:
            logging.error(f"package {package['package_name']} " +
    ...
    for p, a in my_generator:
        ...
        attribute_category = get_attribute_category(p['package_name'],
                                                    a['attribute_name'])
        ...
        # Same treatment if a package/Attribute has more than one category.
            ...
            logging.error(f"{p['package_name']}.{a['attribute_name']} " +
                          "has more than one category defined in " +
                          f"{complementary_files['Config_File']}")
            sys.exit(1)
        # Otherwise, it is added to top level object for it to be generated
        else:
            my_package = obj_package(p['package_name'],
                                     p['package_descr'],
                                     package_to_be_displayed)
            my_category = obj_category(attribute_category[0]['category_name'],
                                       attribute_category[0]['category_to_display'])
            my_attribute = obj_attribute(a['attribute_name'],
                                         a['attribute_descr'],
                                         a['attribute_def'])
            ...
            if my_package.get_to_display() and my_category.get_to_display():
                ...
                logging.debug(f"{my_package.get_name()}.{my_attribute.get_name()} " +
    ...
    def write_pca(package, category, attribute, file):
        ...
        while len(tmp_descr) > 0:
            ...
            if length == display_options['Max_Length']:
                length = min(tmp_descr[:length].rfind(' '),
                             display_options['Max_Length'])
...
def main(args=None):
    ...
    logging.basicConfig(level=logging_options['Level'],
                        format=logging_options['Format'])

    prog_description = "Generates a reStructuredFile from GPRdoc output for " +\
                       "gprtools attributes"
    parser = argparse.ArgumentParser(description=prog_description)

    parser.add_argument('--input',
                        required=True,
                        help='GPRdoc json output')
    ...
    generate(process_jsons(open_json_file(i_filename),
                           open_json_file(complementary_files['Config_File']),
                           i_filename),
             o_filename)
    logging.info("Documentation generated in %.2f ms" % (
