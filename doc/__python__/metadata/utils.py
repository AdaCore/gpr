...
class obj_attribute:
    ...
    def get_def_str(self):
        ...
        if attribute_key_value_translation['index_type',
                                           self.defi['index_type']] != "":
            def_str.append(
                attribute_key_value_translation['index_type',
                                                self.defi['index_type']])

        if attribute_key_value_translation['index_optional',
                                           self.defi['index_optional']] != "":
            def_str.append(
                attribute_key_value_translation['index_optional',
                                                self.defi['index_optional']])

        def_str.append(attribute_key_value_translation['value', self.defi['value']])

        if self.defi['value'] == "LIST" and self.defi['value_is_set']:
            if attribute_key_value_translation['value_case_sensitive',
                                               self.defi['value_case_sensitive']] != "":
                def_str.append(
                    attribute_key_value_translation['value_case_sensitive',
                                                    self.defi['value_case_sensitive']])

        if attribute_key_value_translation['config_concatenable',
                                           self.defi['config_concatenable']]:
            def_str.append(
                attribute_key_value_translation['config_concatenable',
                                                self.defi['config_concatenable']])
