...
# This extension massages the HTML output of Sphinx, so that the pagefind binary can create
# a searchable index. It should only be loaded if pagefind is in the path.
...
def add_custom_ids(app, doctree, docname):
    for section in doctree.traverse(nodes.section):
        ...
        if title_node is not None:
            ...
            # Generate the unique id by combining the slugified title and the line number
