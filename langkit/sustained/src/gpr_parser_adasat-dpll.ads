...
with Gpr_Parser_AdaSAT.Decisions;
with Gpr_Parser_AdaSAT.Formulas; use Gpr_Parser_AdaSAT.Formulas;
...
--  in package `Gpr_Parser_AdaSAT.Helpers`) and call `Solve` with their formula.
--  A custom decision procedure can be given, else the default one will be
--  used (see `Gpr_Parser_AdaSAT.Decisions` for more information).
...
generic
   ...
   is Gpr_Parser_AdaSAT.Decisions.First_Unassigned;
...
package Gpr_Parser_AdaSAT.DPLL is
   function Solve
     (F        : Formula;
