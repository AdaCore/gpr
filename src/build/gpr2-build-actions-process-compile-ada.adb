...
package body GPR2.Build.Actions.Process.Compile.Ada is
   ...
   begin
      ...
      if Self.Global_Config_Pragmas.Is_Defined then
         for J in Attr.Values.First_Index .. Attr.Values.Last_Index - 1 loop
            Cmd_Line.Add_Argument (Attr.Values.Element (J).Text);
         end loop;
   ...
   begin
      ...
      begin
         ...
         for Dep of Deps loop
            --  Configuration pragma file, preprocessor data or symbol
            --  definition file are returned as dependency but are
            --  not sources of the view. Sources are always reported as simple
            --  names while the other types of dependency can be simple names
            --  or full path.
            ...
            if Dep in Simple_Name then
               ...
               begin
                  if not Src.Is_Defined then
                     ...
                     then
                        ...
                        --  without a directory, which we do not support for
                        --  the signature computation: the same preprocessor
                        --  data file could be duplicated across several source
                        --  directories, and we can not know which one was
               ...
               --  Not a source dependency. Add it no matter the nature of the
               --  dependency.

               if not Self.Signature.Add_Input
                 (Artifacts.Files.Create (Path_Name.Create_File (Dep)),
                  Check_Checksums)
               then
                  return;
               end if;
   ...
   is
   begin
      if Self.Ctxt.Is_Externally_Built and then Self.Ctxt.Is_Library then
         --  The ALI file may not be present for externally built libraries,
         --  because object directories are not installed by gprinstall.
         --  The ALI installed in the library directory needs to be parsed
         --  instead.

         Self.ALI_Object :=
           GPR2.Build.ALI_Parser.Create (Self.Lib_Ali_File.Path);
      end if;

      --  The action may not have been executed, or the ALI file may have been
      --  removed since the last execution, so just report an error if the
      --  ALI file is missing.
         ...
         if Self.Inh_From.Is_Defined then
            ...
            begin
               ...
               if Local_O /= Self.Obj_File then
                  ...
                  --  In particular during the call to
                  --  Update_Binds_From_ALI below.
