...
package body GPR2.Build.Actions.Process.Cargo_Support is
   ...
   Mapping : constant array (Positive range <>) of Mapping_Entry :=
     ((US ("aarch64-elf"),
      ...
      (US ("aarch64-qnx"),
       ...
       Needs_Pthread => False),
      (US ("aarch64-vx7r2"),
       (1 => US ("aarch64-wrs-vxworks-rtp"),
        2 => US ("aarch64-wrs-vxworks-dkm")),
       Needs_Pthread => False),
      (US ("arm-elf"),
      ...
      (US ("x86_64-linux"),
       ...
       Needs_Pthread => True),
      (US ("x86_64-vx7r2"),
       (1 => US ("x86_64-wrs-vxworks-rtp"),
        2 => US ("x86_64-wrs-vxworks-dkm")),
       Needs_Pthread => False),
      (US ("x86_64-windows"),
   ...
   function Canonical_Target (View : GPR2.Project.View.Object) return Name_Type
   is (View.Tree.Target (Canonical => True));
   ...
   is
      GPR_Target : constant Name_Type := Canonical_Target (View);
   begin
      for M of Mapping loop
         if M.GPR = String (GPR_Target) then
   ...
   begin
      if Rust_Target_Attr.Is_Defined then
         ...
         return Default_Triple (Canonical_Target (View));
      end if;
   end Rust_Triple;
