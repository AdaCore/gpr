...
package body GPR2.Build.Actions.Process.Link is
      ...
      begin
         ...
         for Obj of Objects loop
            if Self.Tree.Has_Predecessor (Obj)
              and then
                Self.Tree.Predecessor (Obj) in Compile.Ada.Object'Class
            then
               Libgnat := True;

               declare
                  Comp : constant Compile.Ada.Object'Class :=
   ...
   begin
      ...
      for Lib of Self.Library_Dependencies loop
         ...
         begin
            if Pred not in Object'Class then
               --  Not a GPR2 link action (e.g. Cargo_Build): no embedded
               --  objects or standalone binder options to process.
               goto Next_Lib;
            end if;

            declare
               Link : constant Object'Class := Object'Class (Pred);
            begin
               ...
               --    action should have been created to extract this
               --    information.
               ...
               then
                  for Opt of Link.Options_From_Binder loop
                     Traces.Trace
                       ("Adding the link option """
                        & Opt
                        & """ to "
                        & Self.UID.Image
                        & " coming from the static "
                        & " standalone library "
                        & String (Link.View.Library_Name));
      ...
      if not Self.Is_Static_Library then
         ...
         for Lib of Self.Library_Dependencies loop
            declare
               Lib_Path : constant GPR2.Path_Name.Object := Lib.Path;
            begin
               if Lib.Is_Static then

                  --  Add the static archive

                  Cmd_Line.Add_Argument (Lib_Path.String_Value);
               else
                  --  Shared library: add lib directory, rpath and -l flags.
                  --
                  --  Both come from the artifact rather than from the
                  --  producing view: a library built by Cargo sits in
                  --  Cargo's target directory under its crate name, not in
                  --  Library_Dir under Library_Name.

                  declare
                     Lib_Dir : constant Path_Name.Object :=
                       Lib_Path.Containing_Directory;
                     BN      : constant Value_Type := Lib.Link_Name;
                  begin
                     pragma Assert
                       (BN /= "",
                        "The shared library artifact carries no link name: "
                        & Lib_Path.String_Value);
                     Cmd_Line.Add_Argument
                       (Lib_Dir_Opt
                        & String
                            (Lib_Dir.Relative_Path (Self.Working_Directory)));
                     Append_Rpath (Lib_Dir);
                     Dash_l_Opts.Append ("-l" & String (BN));
                  end;
               end if;
            end;
         end loop;
   ...
   begin
      ...
      for Lib of Self.Library_Dependencies loop
         if Lib.Is_Static
           and then not Self.Signature.Add_Input (Lib, Check_Checksums)
         then
            return;
         end if;
      end loop;
      ...
      if Self.Lib_Symbol_File.Is_Defined then
         if not Self.Signature.Add_Input
           (Self.Lib_Symbol_File, Check_Checksums)
         then
            return;
         end if;
      end if;
      ...
      for Input of Self.Tree.Inputs (Self.UID) loop
         if Self.Tree.Has_Predecessor (Input)
            and then Self.Tree.Predecessor (Input)
                     in GPR2.Build.Actions.Process.Link.Partial.Object'Class
         then
            return True;
         end if;
      end loop;
   ...
   is
      ...
      is
         use GNATCOLL.OS;
         use GPR2.Reporter;
         Args   : GNATCOLL.OS.Process.Argument_List := Nm_Cmd;
         Status : Integer;
      ...
      case Kind is
         when Executable =>
            ...
            begin
               ...
               if Output'Length = 0 then
                  ...
                  if GNAT.OS_Lib.Is_Absolute_Path (String (Output)) then
                     Exec := Path_Name.Create_File (Output);
                  else
                     declare
                        Suff : constant Filename_Optional :=
                          Self.Ctxt.Executable_Suffix;
                     begin
                        if Ada.Strings.Fixed.Index (String (Output), ".") = 0
                        then
                           Exec :=
         ...
         when Global_Archive =>
            ...
            begin
               ...
               Self.Is_Static  := True;
               Self.In_Obj     := True;
               Self.Library    := Artifacts.Library.Create_Static
         ...
         when Library =>
            ...
            declare
               function Link_Name return Value_Type;
               --  The base name to link against, without prefix, extension
               --  nor version. Only a shared library has one.

               ---------------
               -- Link_Name --
               ---------------

               function Link_Name return Value_Type is
                  Prefix : constant Value_Type :=
                    Context.Attribute (PRA.Shared_Library_Prefix)
                      .Value.Text;
                  BN     : constant String :=
                    String
                      (Context.Library_Filename (Without_Version => True)
                         .Base_Name);
               begin
                  pragma Assert
                    (GNATCOLL.Utils.Starts_With (BN, String (Prefix)),
                     "The library artifact name doesn't start"
                     & " with the prefix """ & String (Prefix) & """");

                  return
                    Value_Type (BN (BN'First + Prefix'Length .. BN'Last));
               end Link_Name;

            begin
               if Context.Is_Static_Library then
                  Self.Library :=
                    Artifacts.Library.Create_Static
                      (Context.Library_Filename);
               else
                  Self.Library :=
                    Artifacts.Library.Create_Shared
                      (Context.Library_Filename, Link_Name => Link_Name);
               end if;
            end;
   ...
   function Library_Dependencies
     (Self : Object'Class) return Artifacts.Library.Vectors.Vector is
   begin
      return Result : Artifacts.Library.Vectors.Vector do
         for Input of Self.Tree.Inputs (Self.UID) loop
            if Input in Artifacts.Library.Object'Class then
               Result.Append (Artifacts.Library.Object (Input));
            end if;
         end loop;
      end return;
   ...
   overriding function State (Self : Object) return Actions.Action_State is
      Inherited : constant Actions.Action_State := Actions.Object (Self).State;
   begin
      if Inherited /= Actions.Actionable then
         return Inherited;
      end if;

      --  If no object was added to the link action, do not run the action
      if Self.Embedded_Objects.Is_Empty then
         return Actions.Deactivated;
      end if;
...
end GPR2.Build.Actions.Process.Link;
