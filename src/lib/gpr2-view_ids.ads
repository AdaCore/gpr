...
package GPR2.View_Ids is
   ...
   function Create
     ...
     with Pre  => Project_File.Is_Defined
         ...
         Post => Create'Result /= Null_View_Id;
   ...
   function Import (Str : Optional_Name_Type) return View_Id
     with Pre => Is_Valid_Image (Str);
   --  Returns the View_Id object V for which Image (V) = Str. View_Id_Error
   --  is raised in case Str is not a valid view id image.

   function Is_Valid_Image (Str : Optional_Name_Type) return Boolean;
...
private
   ...
   Null_View_Id    : constant View_Id := (Kind => Null_Id);
