...
--  This test exercises the archive-mode of the Link_Options_Extract action:
--  instead of extracting the ".GPR.linker_options" section from a known
--  "o__<lib>.o" member, the action is pointed at the whole library archive
--  and objdump reports the section from whichever member carries it. This is
--  the fallback used for libraries produced by gprbuild1 that do not rely on
--  "o__<lib>.o to insert linker options"
...
procedure Test is
   ...
   begin
      for Root of Tree.Namespace_Root_Projects loop
         --  Scan the whole library archive rather than a single member.

         Link_Extract.Initialize
