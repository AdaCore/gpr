...
package GPR2.Build.Actions.Thread.Write_File is
...
private
   ...
   overriding
   function View (Self : Write_File_Id) return Project.View.Object
   is (Self.Ctxt);

   overriding
   function Action_Class (Self : Write_File_Id) return Value_Type
   is ("Write File");

   overriding
   function Language (Self : Write_File_Id) return Language_Id
   is (No_Language);

   overriding
   function Action_Parameter (Self : Write_File_Id) return Value_Type
   is (Self.Index'Image (2 .. Self.Index'Image'Last));
