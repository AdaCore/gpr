...
package GPR2.Build.Actions.Thread.Trigger is

   type Trigger_Id (<>) is new Actions.Action_Id with private;

   type Object is new Actions.Thread.Object with private;
   --  Action whose Post_Execution dynamically adds a Followup action to the
   --  graph.  Used to test that the scheduler re-enters the graph iterator
   --  after a successful completion (End_Due_To_Failure tracking).
...
private
   ...
   type Trigger_Id is new Actions.Action_Id with record
      Ctxt : GPR2.Project.View.Object;
   end record;

   overriding
   function View (Self : Trigger_Id) return Project.View.Object
   is (Self.Ctxt);

   overriding
   function Action_Class (Self : Trigger_Id) return Value_Type
   is ("Trigger");

   overriding
   function Language (Self : Trigger_Id) return Language_Id
   is (No_Language);

   overriding
   function Action_Parameter (Self : Trigger_Id) return Value_Type
