...
for lib in "libgnat", "libgnarl":
    print(f"BUILD {lib}...")
    bnr.check_output([GPRBUILD, f"-P{adainclude}/{lib}.gpr", "-p", "-q", "--json-summary", "--keep-temp-files", "-XLIBRARY_KIND=relocatable", f"--RTS={local_rts}"])
    ...
    result = bnr.simple_run(["ldd", os.path.join(local_rts, "adalib", f"{lib}{lib_ext}")]).out
